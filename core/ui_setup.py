...
from processing import (detector, tracker, algorithm, save_video_test1)
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.pushButton42.clicked.connect(lambda:save_video_test1.video_record())
    ...
    def turnOnCamera(self):
        ...
        # 读取每帧图片
            ...
            list_bboxs = []
            list_bboxs2 = []
            bboxes2 = detector.Detector.detect(im2)
            ...
            if len(bboxes2) > 0:
                list_bboxs2 = tracker.update(bboxes2, im2)
                output_image_frame2 = tracker.draw_bboxes(im2, list_bboxs2, line_thickness=None)
            else:
                output_image_frame2 = im2
            ...
            output_image_frame2 = cv2.resize(output_image_frame2, (camera.color_polygons_image.shape[1], camera.color_polygons_image.shape[0]))

            # # 执行图像相加操作
            output_image_frame = cv2.add(output_image_frame, camera.color_polygons_image)
            output_image_frame2 = cv2.add(output_image_frame2, camera.color_polygons_image)
